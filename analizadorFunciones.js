...
const operadoresAritmeticos = new Set(['+', '-', '/', '*', 'DIV', 'MOD']);
...
const palabrasReservadas = new Set([
    ...
    'SET', 'STRING', 'TRY', 'TYPE', 'UNIT', 'USES', 'VAR', 'WITH', 'WRITELN', 'CONST'
]);
const simbolosPuntuacion = new Set([':', ';', ',', '.', '(', ')', '[', ']', '{', '}']);
const tiposDatos = new Set(['BOOLEAN', 'CHAR', 'STRING', 'WORD', 'INTEGER', 'BYTE', 'REAL', 'LONGINT', 'SHORTINT']);

/**
 * Verifica si un token es un operador aritmético válido en Pascal.
 */
export function esOperadorAritmetico(token) {
    return esTokenValido(token.toUpperCase(), operadoresAritmeticos);
}
